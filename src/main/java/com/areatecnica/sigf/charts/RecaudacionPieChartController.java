 ...
 */
package com.areatecnica.sigf.charts;
 ...
 * FIN OBSERVACIONES ***
 */
public class RecaudacionPieChartController extends AbstractCrudController<Recaudacion> implements IProcess {
     ...
     */
    public RecaudacionPieChartController() {
    ...
    public void load() {
        ...
        for (CajaRecaudacion caja : this.cajaRecaudacionItems) {
            this.recaudacionDao = new IRecaudacionDaoImpl();
            this.setItems(this.recaudacionDao.findByCajaFechaRecaudacion(caja, getFechaRecaudacion()));
        ...
        if (!this.columnModelItems.isEmpty()) {
            for (Column c : this.columnModelItems) {
                System.err.println("VALOR:" + c.getValor() + " ITEM:" + c.getNombre());
                this.pieModel.set(c.getNombre(), c.getValor());
            }
        } else {
            this.pieModel.set("Sin Recaudación", 0);
        }

        this.pieModel.setTitle("Total Recaudado");
