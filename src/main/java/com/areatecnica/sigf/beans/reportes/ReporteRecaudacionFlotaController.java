 ...
 * FIN OBSERVACIONES ***
 */
public class ReporteRecaudacionFlotaController extends AbstractCrudController<Recaudacion> implements IProcess {
    ...
    private LinkedHashMap _defaultLink;

    /**
     * Creates a new instance of RecaudacionGeneralController
     */
    public ReporteRecaudacionFlotaController() {
    ...
    public void init() {
        ...
        this.flotaItems = ((List<Flota>) this.flotaDao.findByCuenta(this.getUserCount()));

        this.setTotalRecaudacion(0);
        this.setTotalCombustible(0);
        this.setTotalMinutos(0);
        this.setEgresoItems(new ArrayList<>());
        CajaRecaudacion caja = this.getCurrentUser().getCajaRecaudacionList().get(0);
        this.egresoCajaDao = new IEgresoCajaRecaudacionDaoImpl();
        List<EgresoCajaRecaudacion> auxEgreso = this.egresoCajaDao.findAllByCajaRecaudacion(caja);
        _defaultLink = new LinkedHashMap();
    ...
    /**
     * @param flotaItems the flotaItems to set
     */
    ...
    public void load() {
        ...
        for (Map.Entry<String, Egreso> a : header.entrySet()) {
            ...
            System.err.println("LLAVE:"+a.getKey());
            if (getTotals().containsKey(a.getKey())) {
                int aux = (int) getTotals().get(a.getKey());
                resultsTotals.add(getFormatValue(aux));
            } else {
                resultsTotals.add(getFormatValue(0));
            }
    ...
    public void handleFlotaChange(ActionEvent event) {
        if (this.getFlota() != null) {
            this.busItems = this.getFlota().getBusList();
        }
    }
